...
export const words = pgTable(
	...
	(t) => [index('ux_words_french_normalized').on(t.frenchNormalized)]
);
...
export const adminLists = pgTable(
	...
	(t) => [index('idx_admin_lists_name').on(t.name)]
);
...
export const userWords = pgTable(
	...
	(t) => [
		index('ux_user_words_user_word').on(t.userId, t.wordId),
		index('idx_user_words_active').on(t.userId, t.active),
		index('idx_user_words_last_seen').on(t.userId, t.lastSeen)
	]
);
...
export const exposures = pgTable(
	...
	(t) => [
		...
		index('idx_exposures_recent').on(t.userId, t.lastSeen)
	]
);
...
export const practiceLogs = pgTable(
	'practice_logs',
	{
		...
		wordId: uuid('word_id')
			.notNull()
			.references(() => words.id, { onDelete: 'cascade' }),
		sourceListId: uuid('source_list_id').references(() => adminLists.id, { onDelete: 'set null' }),
	...
	(t) => [
		index('idx_practice_logs_user_time').on(t.userId, t.createdAt),
