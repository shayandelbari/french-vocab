...
import { userWords, words } from '$lib/server/db/schema';
import { eq, and } from 'drizzle-orm';
import { json } from '@sveltejs/kit';
import { wordInsertSchema } from '$lib/server/db/schema';
...
export async function GET({ locals }: RequestEvent) {
	...
	const myWords = await db
		.select({
			...
			addedAt: userWords.addedAt,
			notes: userWords.notes,
			seenCount: userWords.seenCount,
			lastSeen: userWords.lastSeen,
			active: userWords.active
		})
		.from(userWords)
		.innerJoin(words, eq(userWords.wordId, words.id))
		.where(and(eq(userWords.userId, userId), eq(userWords.active, true)))
		.orderBy(userWords.addedAt);

	return json({ success: true, data: myWords });
}
